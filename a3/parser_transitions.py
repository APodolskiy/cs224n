...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.dependencies = list()  # type: List[Tuple]
        ### END YOUR CODE
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            raise ValueError(f"Wrong transition: {transition}")
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        ...
        transitions = model.predict(partial_parses=batch)
        for parse, trans in zip(batch, transitions):
            parse.parse_step(trans)
        # check if finished
        unfinished_parses = [item for item in unfinished_parses if not item.is_parsed()]
    ### END YOUR CODE
